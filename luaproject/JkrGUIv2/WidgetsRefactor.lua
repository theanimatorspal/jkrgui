...
Jkr.CreateWidgetRenderer = function(i, w, e)
    ...
    o.CreateSampledImage = function(inPosition_3f, inDimension_3f, inFileName, inNoDraw, inColor)
        ...
        if (not inNoDraw) then
            SampledImage.DrawId = o.c.Push(Jkr.CreateDrawable(SampledImage.imageViewRect, false, "IMAGE",
                SampledImage.mId,
                SampledImage.mColor))
        end
    ...
    o.CreateComputeImage = function(inPosition_3f, inDimension_3f, inOptCompatibleSampledImage)
        ...
        if inOptCompatibleSampledImage then
            ComputeImage.mId = Jkr.CreateCustomPainterImage(i, w, math.int(inOptCompatibleSampledImage.mActualSize.x),
                math.int(inOptCompatibleSampledImage.mActualSize.y))
        else
            ComputeImage.mId = Jkr.CreateCustomPainterImage(i, w, math.int(inDimension_3f.x),
                math.int(inDimension_3f.y))
        end

        ComputeImage.RegisterPainter = function(inPainter)
            ComputeImage.mId:Register(i, inPainter.handle)
        end
    ...
    o.Draw = function()
        ...
        for i = 1, #o.c.mDrawables, 1 do
            ...
            if drawable.mDrawType == "IMAGE" then
                ...
                o.s:Draw(w, drawable.mColor, drawable.mId, drawable.mId, o.UIMatrix,
                    Jkr.CmdParam.UI)
            end
        end
    end
