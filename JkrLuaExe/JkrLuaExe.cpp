...
void CreateMainBindings(sol::state& s) {
	     ...
	     Jkr.new_usertype<Jkr::Instance>(
		   "Instance", sol::call_constructor, sol::factories([](int inDescriptorSize, int inPoolSize) {
	     ...
	     Jkr.new_usertype<Jkr::WindowMulT>(
		   ...
		   sol::factories(
			 [](Jkr::Instance& inInstance, std::string_view inTitle, int inWidth, int inHeight) {
	     ...
	     struct ShapeRendererResources {
			void Add(Jkr::Instance& inInstance, FillType inFillType,
			         ...
			         std::string_view inComputeShader, bool inShouldStore) {
		   ...
		   sol::factories([](Jkr::Instance& inInstance,
				 ...
				 int invardescount) {
			        return mu<Jkr::Renderer::Shape>(
				      inInstance, inCompatibleWindow, inResources.mCaches, invardescount);
		   }));
