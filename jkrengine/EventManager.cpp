...
void Jkr::EventManager::ProcessEventsEXT(Window &inWindow) {
                    ...
                    mOffscreenByWindowSize =
                         glm::vec2(mFrameSize.x / mWindowSize.x, mFrameSize.y / mWindowSize.y);
                    mCloseWindowEvent = false;

                    while (SDL_PollEvent(&mEvent)) {
                                        if (mEvent.type == SDL_WINDOWEVENT and
...
bool Jkr::EventManager::IsMouseWithinAtTopOfStack(uint32_t inId, uint32_t inDepthValue) {
                    for (auto &mapElement : mBoundRect2Ds) {
                                        if (mapElement.first < inDepthValue) {
                                                            for (auto &vecElement :
                                                                 mBoundRect2Ds[mapElement.first]) {
                                                                                if (vecElement.IsPointWithin(
                                                                                         mMousePos,
                                                                                         mOffscreenByWindowSize))
                                                                                                    return false;
                                                            }
...
void Jkr::EventManager::UpdateBoundedRect(uint32_t inId,
                    ...
                    try {
                                        mBoundRect2Ds[inDepthValue][inId] =
                                             BoundRect2D{.mXy = inXy, .mWh = inWh};
                    } catch (std::exception &e) {
                                        ksai::Log(e.what(), "ERROR");
                    }
}
void Jkr::EventManager::RemoveBoundedRect(uint32_t inId, uint32_t inDepthValue) {
                    auto &vec = mBoundRect2Ds[inDepthValue];
                    if (inId < vec.size()) {
                                        vec.erase(vec.begin() + inId);
                    }
}
...
bool Jkr::EventManager::IsLeftButtonPressed() const {
                    return (mEvent.type == SDL_MOUSEBUTTONDOWN) &&
...
glm::vec4 Jkr::EventManager::GetMultiGesture() const {
                    return glm::vec4(mEvent.mgesture.x,
                                     mEvent.mgesture.y,
                                     mEvent.mgesture.dDist,
                                     mEvent.mgesture.dTheta);
}