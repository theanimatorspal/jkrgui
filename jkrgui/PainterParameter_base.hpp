...
namespace Jkr {
enum class PainterParameterContext { StorageBuffer, UniformBuffer, StorageImage, UniformImage, SkyboxImage, UniformSampler };

using StorageBufferType = VulkanBufferVMA<BufferContext::Storage, MemoryType::DeviceLocal>;
using StorageBufferTypeCoherent = VulkanBufferVMA<BufferContext::Storage, MemoryType::HostVisibleAndCoherenet>;
using UniformBufferType = VulkanBufferVMA<BufferContext::Uniform, MemoryType::HostVisibleAndCoherenet>;
using StorageAndUniform = VulkanBufferVMA<BufferContext::Storage | BufferContext::Uniform, MemoryType::HostVisibleAndCoherenet>;
...
class PainterParameterBase {
       ...
       public:
               ...
               void Setup(Up<StorageBufferTypeCoherent>& inUniformBuffer, vk::DeviceSize inDeviceSize, void** inMappedMemoryRegion);
       ...
       private:
               template <typename T> void SetupImage(Up<VulkanSampler>& inUniformImageSampler, Up<T>& inUniformImage, std::span<const sv> inFileNames);
};
} // namespace Jkr

namespace Jkr {
template <typename T> inline void PainterParameterBase::SetupImage(Up<VulkanSampler>& inUniformImageSampler, Up<T>& inUniformImage, std::span<const sv> inFileNames) {
               int Width;
               int Height;
               int Channels;
               ...
               inUniformImage->SubmitImmediateCmdCopyFromData(mInstance.GetGraphicsQueue(), mInstance.GetUtilCommandBuffer(), mInstance.GetDevice(), Channels * Height * Width, Datas_ptr);
