...
public:
    ...
    void Draw(const Primitive& inPrimitive,
        ...
        const Window& inWindow, CmdParam inCmdParam = CmdParam::UI);
    template <class PushType>
    ...
    static void BindDrawParamtersVertexAndIndexBuffersOnly_EXT(const Instance &inInstance,
                                                               ...
                                                               const Window &inWindow, CmdParam inCmdContext = CmdParam::UI);
    ...
    void Draw_EXT(const Primitive& inPrimitive,
        ...
        uint32_t inFirstInstance, CmdParam inCmdContext = CmdParam::UI);
    template <class PushType>
    void Draw_EXT(const Primitive& inPrimitive,
        const vk::ArrayProxy<PushType> inPushConstants,
        uint32_t inIndexCount,
    ...
    void Dispatch(const vk::ArrayProxy<PushType> inPushConstants,
        uint32_t inCountX,
        uint32_t inCountY,
        uint32_t inCountZ, CmdParam inCmdContext = CmdParam::UI);
    ...
    static void OptimizeParameter(const Instance& inInstance, const PainterParameter<PainterParameterContext::StorageImage>& inImage,
        const Window& inWindow);
...
inline void Painter::BindDrawParamtersDescriptorsOnly_EXT(const Primitive &inPrimitive,
                                                          const Window &inWindow, CmdParam inCmdParam)
{
...
void Painter::Dispatch_EXT(const vk::ArrayProxy<PushType> inPushConstants,
                           uint32_t inCountX,
                           uint32_t inCountY,
                           uint32_t inCountZ, CmdParam inCmdParam)
{
...
void Painter::Draw_EXT(const Primitive& inPrimitive,
    ...
    uint32_t inInstanceCount,
    uint32_t inFirstIndex,
    uint32_t inFirstInstance, CmdParam inCmdParam)
{
    auto& Cmd = inWindow.GetCommandBuffers(inCmdParam)[inWindow.GetCurrentFrame()];
