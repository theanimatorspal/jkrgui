...
class Painter {
       ...
       public:
               ...
               static void BindDrawParamtersVertexAndIndexBuffersOnly_EXT(const Instance& inInstance, const Primitive& inPrimitive, const Window& inWindow, CmdParam inCmdContext = CmdParam::UI);
               ...
               template <class PushType> void Dispatch(const vk::ArrayProxy<PushType> inPushConstants, uint32_t inCountX, uint32_t inCountY, uint32_t inCountZ, CmdParam inCmdContext = CmdParam::UI);
               ...
               template <class PushType>
               void Dispatch_EXT(const vk::ArrayProxy<PushType> inPushConstants, uint32_t inCountX, uint32_t inCountY, uint32_t inCountZ, CmdParam inCmdContext = CmdParam::UI);
               ...
               static void OptimizeParameter(const Instance& inInstance, const PainterParameter<PainterParameterContext::StorageImage>& inImage, const Window& inWindow);

       private:
               static void
               OptimizeImageParameter(const Instance& inInstance, const VulkanCommandBuffer& inBuffer, const PainterParameter<PainterParameterContext::StorageImage>& inImage, const Window& inWindow);
...
inline void Painter::BindDrawParamtersVertexAndIndexBuffersOnly_EXT(const Instance& inInstance, const Primitive& inPrimitive, const Window& inWindow, CmdParam inCmdParam) {
...
template <class PushType> void Painter::Dispatch_EXT(const vk::ArrayProxy<PushType> inPushConstants, uint32_t inCountX, uint32_t inCountY, uint32_t inCountZ, CmdParam inCmdParam) {
...
template <class PushType> void Painter::Dispatch(const vk::ArrayProxy<PushType> inPushConstants, uint32_t inCountX, uint32_t inCountY, uint32_t inCountZ, CmdParam inCmdParam) {
               auto& Cmd = mWindow.GetCommandBuffers(inCmdParam)[mWindow.GetCurrentFrame()];
               Cmd.GetCommandBufferHandle().bindDescriptorSets(
...
template <class PushType> void Painter::Draw(const Primitive& inPrimitive, const vk::ArrayProxy<PushType> inPushConstants) { Draw(inPrimitive, inPushConstants, mWindow); }

template <class PushType> void Painter::Draw(const Primitive& inPrimitive, const vk::ArrayProxy<PushType> inPushConstants, const Window& inWindow, CmdParam inCmdParam) {
               auto& Cmd = mWindow.GetCommandBuffers(inCmdParam)[inWindow.GetCurrentFrame()];
               inPrimitive.GetVertexBufferPtr()->Bind<BufferContext::Vertex>(Cmd);
               inPrimitive.GetIndexBufferPtr()->Bind<BufferContext::Index>(Cmd);
               Cmd.GetCommandBufferHandle().bindDescriptorSets(
