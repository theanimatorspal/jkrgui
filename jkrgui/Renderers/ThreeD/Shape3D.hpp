...
class Shape : public Renderer_base, public Shape_base {
      ...
      public:
          ...
          std::unique_lock<std::mutex> MakeLockedLocker() { return std::unique_lock<std::mutex>(mPainterMutex); }
      ...
      public:
          ...
          */
          template <typename T> void WriteToGlobalUB(T inData);
          ...
          */
          ...
          template <typename T> void PainterDraw(ui inId, T inPush, Window& inWindow, ComPar inCompar);
          template <typename T> void PainterDraw(ui inPipelineId, ui inId, T inPush, Window& inWindow, ComPar inCompar);
...
template <typename T> void Shape::WriteToGlobalUB(T inData) {
          T d = inData;
...
template <typename T> void Shape::PainterDraw(ui inId, T inPush, Window& inWindow, ComPar inComPar) {
          mPainters[mBoundPainterVF]->Draw_EXT<T>(*mPrimitive, inPush, inWindow, gb::GetIndexCount(inId), 1, 0, 0, inComPar);
}

template <typename T> inline void Shape::PainterDraw(ui inPipelineId, ui inId, T inPush, Window& inWindow, ComPar inComPar) {
          mPainters[inPipelineId]->Draw_EXT<T>(*mPrimitive, inPush, inWindow, gb::GetIndexCount(inId), 1, 0, 0, inComPar);
}

inline void Shape::Bind(Window& inWindow, ComPar inCompar) { Painter::BindDrawParamtersVertexAndIndexBuffersOnly_EXT(mInstance, *mPrimitive, inWindow, inCompar); }

inline void Shape::RegisterGlobalUBToPainter(ui inPainterId, sz inOffset, ui inBinding, ui inDestinationArrayElement) {
...
inline void Shape::RegisterGlobalSSBOToPainter(ui inPainterId, sz inOffset, ui inBinding, ui inDestinationArrayElement) {
          mPainters[inPainterId]->RegisterPainterParameter(*mGlobalSSBO, inOffset, inBinding, inDestinationArrayElement, RegisterMode::AllShaders);
}
...
inline void Shape::AddModelTextureToPainter(ui inId, ui inPainterId, sz inOffset, ui inBinding, ui inDestinationArrayElement) {
...
inline void Shape::RegisterExternalTextureToPainter(ui& outId, ui inPainterId, const sv inFilename, sz inOffset, ui inBinding, ui inDestinationArrayElement) {
          ...
          mPainters[inPainterId]->RegisterPainterParameter(*mExternalTextures.back(), inOffset, inBinding, inDestinationArrayElement, RegisterMode::VertexFragmentOnly);
}

inline void Shape::RegisterExternalTextureArrayToPainter(ui& outId, ui inPainterId, std::span<const sv> inFileNames, sz inOffset, ui inBindingn, ui inDestinationArrayElement) {
          Up<ImageType> AnImage = MakeUp<ImageType>(mInstance);
          AnImage->Setup(inFileNames);
          mExternalTextures.push_back(mv(AnImage));
          outId = mExternalTextures.size() - 1;
          mPainters[inPainterId]->RegisterPainterParameter(*mExternalTextures.back(), inOffset, inBindingn, inDestinationArrayElement, RegisterMode::VertexFragmentOnly);
}
