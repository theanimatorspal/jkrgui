...
CustomImagePainter::CustomImagePainter(sv inName, sv inComputeShaderFunction, sv inPushConstantSignature, ui inX, ui inY, ui inZ)
    : mCustomPainterFileName(std::string(inName)), mThreads(inX, inY, inZ) {
               ...
               mComputeStream << "layout(local_size_x = " << inX << ", local_size_y = " << inY << "," << " local_size_z = " << inZ << ") in;";
...
void CustomImagePainter::Store(const Instance& inInstance, Window& inWindow) {
               mCustomPainterCache = MakeUp<PainterCache>(inInstance);
               mCustomPainterCache->Store(mCustomPainterFileName, mVertexStream.str(), mFragmentStream.str(), mComputeStream.str());
               mPainter = MakeUp<Painter>(inInstance, inWindow, *mCustomPainterCache);
}

CustomImagePainter::CustomImagePainter(sv inName, sv inComputeShader) : mCustomPainterFileName(std::string(inName)) {
