...
class Shape : public Shape_base, Renderer_base {
	     ...
	     using CopyCommandType = std::function<void(const ksai::VulkanCommandBuffer&, ksai::VulkanImageBase&,
					        ksai::VulkanImageBase&)>;
	   ...
	   public:
		 ...
		 std::unordered_map<FillType, Up<PainterCache>>& inPainterCaches, uint32_t inVarDesCount = 5000);
	     ...
	     void Draw(Window& inWindow, glm::vec4 inColor, uint32_t inWindowW, uint32_t inWindowH,
		     uint32_t inStartShapeId, uint32_t inEndShapeId, glm::mat4 inMatrix);
