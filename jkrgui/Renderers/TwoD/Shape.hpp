...
class Shape : public Shape_base, Renderer_base {
       ...
       public:
               Shape(const Instance& inInstance, std::unordered_map<FillType, Up<PainterCache>>& inPainterCaches) : mInstance(inInstance), mPainterCaches(inPainterCaches) {}
               Shape(const Instance& inInstance, Window& inCompatibleWindow, std::unordered_map<FillType, Up<PainterCache>>& inPainterCaches, uint32_t inVarDesCount = 5000);
               ...
               void DrawEXT(Window& inWindow, glm::vec4 inColor, ui inStartShapeId, ui inEndShapeId, glm::mat4 inMatrix, CmdParam inParam);
...
inline void Jkr::Renderer::Shape::UpdateEXT(ui inId, Jkr::Generator& inShape, glm::vec3 inPosition) { Update(inId, inShape, inPosition.x, inPosition.y, inPosition.z); }

inline void Jkr::Renderer::Shape::DrawEXT(Window& inWindow, glm::vec4 inColor, ui inStartShapeId, ui inEndShapeId, glm::mat4 inMatrix, CmdParam inParam) {
               PushConstant Push;
               Push.mColor  = inColor;
