...
Shape::Shape(const Instance& inInstance,
    ...
    : mInstance(inInstance), mPainterCaches(inPainterCaches) {
          ...
          mVarDesVulkanDescriptorSet = MakeUp<VulkanDescriptorSet>(mInstance.GetDevice(),
          ...
          rb::CreateStagingBuffers(
           inInstance, sb::VertexCountToBytes(mTotalNoOfVerticesRendererCanHold), sb::IndexCountToBytes(mTotalNoOfIndicesRendererCanHold));
#endif
...
void Shape::Add(Jkr::Generator& inShape, int inX, int inY, ui inZ, ui& outId) {
          ...
          rb::RegisterBufferCopyRegionToPrimitiveFromStaging(sb::VertexCountToBytes(sb::GetVertexOffsetAbsolute(OffsetId)),
                                                             sb::IndexCountToBytes(sb::GetIndexOffsetAbsolute(OffsetId)),
                                                             sb::VertexCountToBytes(inShape.GetVertexCount()),
                                                             sb::IndexCountToBytes(inShape.GetIndexCount()));
#else
...
void Shape::AddImage(const std::string_view inFileName, ui& outIndex) {
          ...
          Up<VulkanDescriptorSet> Desset = MakeUp<VulkanDescriptorSet>(
           mInstance.GetDevice(), mInstance.GetDescriptorPool(), mPainterCaches[FillType::Image]->GetVertexFragmentDescriptorSetLayout());
...
void Shape::CopyToImage(ui inId, ui inWidth, ui inHeight, CustomPainterImage& inPainterImage) {
          ...
          mImageCopyCommands.push_back(
           [&](const ksai::VulkanCommandBuffer& inCmd, ksai::VulkanImageBase& inSrcImage, ksai::VulkanImageBase& inDstImage) {
                     ...
                     vk::ImageCopy ImageCopy(
                      SrcSubresource, vk::Offset3D(0, 0, 0), DstSubresource, vk::Offset3D(0, 0, 0), vk::Extent3D(srcImgProp.mExtent, 1));
...
void Shape::Draw(Window& inWindow, glm::vec4 inColor, ui inWindowW, ui inWindowH, ui inStartShapeId, ui inEndShapeId, glm::mat4 inMatrix) {
          glm::mat4 Matrix = glm::ortho(0.0f, static_cast<float>(inWindowW), 0.0f, static_cast<float>(inWindowH), 100.0f, -100.0f) * inMatrix;
          PushConstant Push;
          Push.mColor = inColor;
          Push.mMatrix = Matrix;
...
void Shape::CheckAndResize(const Jkr::Generator& inShape) {
          ...
          while (true) {
                    bool ResizeRequiredi = (inShape.GetIndexCount() + sb::GetCurrentIndexOffset() >= mTotalNoOfIndicesRendererCanHold) ||
          ...
          if (ResizeRequired) {
                    ...
                    mPrimitive.reset();
                    mPrimitive = MakeUp<Primitive>(
                     mInstance, sb::VertexCountToBytes(mTotalNoOfVerticesRendererCanHold), sb::IndexCountToBytes(mTotalNoOfIndicesRendererCanHold));
#ifndef JKR_NO_STAGING_BUFFERS
                    ...
                    rb::CopyToStagingBuffers(sb::GetVertexBufferData(),
                                             ...
                                             sb::VertexCountToBytes(mTotalNoOfVerticesRendererCanHold),
                                             sb::IndexCountToBytes(mTotalNoOfIndicesRendererCanHold));
                    rb::RegisterBufferCopyRegionToPrimitiveFromStaging(sb::VertexCountToBytes(0),
                                                                       sb::IndexCountToBytes(0),
                                                                       sb::VertexCountToBytes(mTotalNoOfVerticesRendererCanHold),
                                                                       sb::IndexCountToBytes(mTotalNoOfIndicesRendererCanHold));
#else
