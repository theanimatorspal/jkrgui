...
class CustomImagePainter {
        ...
        public:
          ...
          void RegisterImage(const Instance& inInstance, Window& inWindow, CustomPainterImage& inImage) { mPainter->RegisterPainterParameter(inImage.GetPainterParam(), 0, 0, 0, Jkr::RegisterMode::ComputeOnly); }
          ...
          void BindImageFromImage(const Window& inWindow, CustomPainterImage& inImage, ComPar inPar = ComPar::None) {
                    ...
                    Cmd.GetCommandBufferHandle().bindDescriptorSets(vk::PipelineBindPoint::eCompute, mCustomPainterCache->GetComputePipelineLayout().GetPipelineLayoutHandle(), 0, inImage.mVulkanDescriptorSet->GetDescriptorSetHandle(), {});
          }
          ...
          template <class T> void Draw(Window& inWindow, T inPushConstant, ComPar inPar = ComPar::None) { mPainter->Dispatch_EXT<T>(inPushConstant, mThreads.x, mThreads.y, mThreads.z, inPar); }
