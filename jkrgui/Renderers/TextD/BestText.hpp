...
public:
    ...
    bb::TextDimensions UpdateText(uint32_t inId, const std::string_view inText,
        uint32_t inX, uint32_t inY,
        uint32_t inDepthValue);
    void Dispatch(Window& inWindow);
    void Bind(Window& inWindow);
    void Draw(Window& inWindow, glm::vec4 inColor, uint32_t inWindowW,
        uint32_t inWindowH, uint32_t inStartTextId, uint32_t inNoOfChars,
        glm::mat4 inMatrix);
...
public:
    ...
    SETTER SetCurrentFontFace(uint32_t inFontFaceId) { mCurrentFontFaceId = inFontFaceId; }
