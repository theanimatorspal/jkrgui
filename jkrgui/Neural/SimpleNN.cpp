                    ...
                    if (i > 0) {
                              if (i != mTopology.size() - 1) {
                                        ...
                                        mWeights.emplace_back(
                                         matX(mTopology[i - 1] + 1, mTopology[i])); // bias for left and not for right(last layer)
                                        mWeights.back().setRandom();
                              }
                    }
          }
                    ...
                    if (i != mTopology.size() - 2) {
                              for (uint32_t c = 0; c < mWeights[i].cols() - 1; c++) {
                                        for (uint32_t r = 0; r < mWeights[i].rows(); r++) {
                                                  mWeights[i].coeffRef(r, c) += mLearningRate * mDeltas[i + 1].coeffRef(c) *
...
template <typename T, Op inShouldWrite = Op::Write> inline static T RW(std::fstream& inFile, std::optional<T> inType = std::nullopt) {
...
template <typename T, Op inShouldWrite = Op::Write> inline static void RW(std::fstream& inFile, IsVector auto& outType) {
          if constexpr (inShouldWrite == Op::Write) {
