...
ShaderCompiler::ShaderCompiler(const std::string_view inVertexShaderString,
    ...
    if (inVertexShaderString != "" and inFragmentShaderString != "") {
        bool success =
             SpirvHelper::GLSLtoSPV(vk::ShaderStageFlagBits::eVertex, inVertexShaderString.data(), outVertexShaderModule);
        if (!success) std::cout << "VertexShader Failed=============================\n";
        success =
             SpirvHelper::GLSLtoSPV(vk::ShaderStageFlagBits::eFragment, inFragmentShaderString.data(), outFragmentShaderModule);
        if (!success) std::cout << "FragmentShader Failed=============================\n";
    }
}

ShaderCompiler::ShaderCompiler(const std::string_view inComputeShaderString, std::vector<uint32_t> &outComputeShaderModule) {
    if (inComputeShaderString != "") {
        bool success =
             SpirvHelper::GLSLtoSPV(vk::ShaderStageFlagBits::eCompute, inComputeShaderString.data(), outComputeShaderModule);
...
Jkr::ShaderModules::ShaderModules(const VulkanDevice &inDevice,
    ...
    if (inVertexShaderSPIRV.size() > 0 and inFragmentShaderSPIRV.size() > 0) {
        mVertexFragmentShaderModules.push_back(VulkanShaderModule(inDevice, inVertexShaderSPIRV, false));
        mVertexFragmentShaderModules.push_back(VulkanShaderModule(inDevice, inFragmentShaderSPIRV, false));
    }
...
Jkr::PainterCache &Jkr::PainterCache::Load(const std::string &fileName, ui inVardescount) {
    ...
    mShaderModulesPtr        = MakeUp<ShaderModules>(
         mInstance.GetDevice(), mVertexFragmentShaderSPIRV[0], mVertexFragmentShaderSPIRV[1], mComputeShaderSPIRV[0]);

    time.reset();
    mPtrVertexFragmentDescriptorSetLayout =
         MakeUp<VulkanDescriptorSetLayout<2, ShaderModuleContext::Vertex, ShaderModuleContext::Fragment>>(
              mInstance.GetDevice(), mShaderModulesPtr->GetShaderModulesArray());
    mPtrVertexFragmentPipelineLayout = MakeUp<VulkanPipelineLayout<2>>(
         mInstance.GetDevice(), *mPtrVertexFragmentDescriptorSetLayout, mShaderModulesPtr->GetShaderModulesArray());
    mPtrPipelineCache              = MakeUp<VulkanPipelineCache>(mInstance.GetDevice(), fileName);

    mPtrComputeDescriptorSetLayout = MakeUp<VulkanDescriptorSetLayout<1, ShaderModuleContext::Compute>>(
         mInstance.GetDevice(), mShaderModulesPtr->GetComputeShaderModuleArray());
    mPtrComputePipelineLayout = MakeUp<VulkanPipelineLayout<1>>(
         mInstance.GetDevice(), *mPtrComputeDescriptorSetLayout, mShaderModulesPtr->GetComputeShaderModuleArray());
...
#else
Jkr::PainterCache &Jkr::PainterCache::Load(const std::string &fileName, uint32_t inVariableDescirptorCount) {
    time.reset();
    LoadSPIRVsFromFile(fileName);
    ...
    mPtrVertexFragmentDescriptorSetLayout =
         MakeUp<VulkanDescriptorSetLayout<2, ShaderModuleContext::Vertex, ShaderModuleContext::Fragment>>(
              mInstance.GetDevice(), mShaderModulesPtr->GetShaderModulesArray(), inVariableDescirptorCount);
    mPtrVertexFragmentPipelineLayout = MakeUp<VulkanPipelineLayout<2>>(
         mInstance.GetDevice(), *mPtrVertexFragmentDescriptorSetLayout, mShaderModulesPtr->GetShaderModulesArray());
    mPtrPipelineCache              = MakeUp<VulkanPipelineCache>(mInstance.GetDevice(), fileName);

    mPtrComputeDescriptorSetLayout = MakeUp<VulkanDescriptorSetLayout<1, ShaderModuleContext::Compute>>(
         mInstance.GetDevice(), mShaderModulesPtr->GetComputeShaderModuleArray(), inVariableDescirptorCount);
    mPtrComputePipelineLayout = MakeUp<VulkanPipelineLayout<1>>(
         mInstance.GetDevice(), *mPtrComputeDescriptorSetLayout, mShaderModulesPtr->GetComputeShaderModuleArray());
    time.elapsed("Shader Descriptor + Pipeline Reflection");
...
Jkr::PainterCache &Jkr::PainterCache::Store(const std::string &fileName,
    ...
    mPtrVertexFragmentDescriptorSetLayout =
         MakeUp<VulkanDescriptorSetLayout<2, ShaderModuleContext::Vertex, ShaderModuleContext::Fragment>>(
              mInstance.GetDevice(), mShaderModulesPtr->GetShaderModulesArray(), inVariableDescriptorCount);
    mPtrVertexFragmentPipelineLayout = MakeUp<VulkanPipelineLayout<2>>(
         mInstance.GetDevice(), *mPtrVertexFragmentDescriptorSetLayout, mShaderModulesPtr->GetShaderModulesArray());
    mPtrPipelineCache              = MakeUp<VulkanPipelineCache>(mInstance.GetDevice(), fileName);

    mPtrComputeDescriptorSetLayout = MakeUp<VulkanDescriptorSetLayout<1, ShaderModuleContext::Compute>>(
         mInstance.GetDevice(), mShaderModulesPtr->GetComputeShaderModuleArray(), inVariableDescriptorCount);
    mPtrComputePipelineLayout = MakeUp<VulkanPipelineLayout<1>>(
         mInstance.GetDevice(), *mPtrComputeDescriptorSetLayout, mShaderModulesPtr->GetComputeShaderModuleArray());
    time.elapsed("Shader Descriptor + Pipeline Reflection");
