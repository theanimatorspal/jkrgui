...
class Uniform3D {
    ...
    using ImageType =
         Jkr::PainterParameter<Jkr::PainterParameterContext::UniformImage>;
    using UniformBufferType =
         Jkr::PainterParameter<Jkr::PainterParameterContext::UniformBuffer>;
    using StorageBufferType =
         Jkr::PainterParameter<Jkr::PainterParameterContext::StorageBuffer>;
    ...
    static Up<Uniform3D>
    CreateByGLTFNodeIndex(const Instance& inInstance,
                          Simple3D& inSimple3D,
    ...
    void AddTextureByVector(int inDstBinding,
                            v<uc>& inImageVector,
                            ui inWidth,
                            ui inHeight);
    ...
    Uniform3D(const Instance& inInstance, Simple3D& inSimple3D)
...
template <typename T>
inline void Uniform3D::UpdateUniformBuffer(int inDstBinding, T inData) {
    T data = inData;
    void* memory =
         mUniformBuffers[inDstBinding]->GetUniformMappedMemoryRegion();
    memcpy(memory, &data, sizeof(T));
}

template <typename T>
inline void Uniform3D::UpdateStorageBuffer(int inDstBinding, T inData) {
    T data = inData;
    void* memory =
         mStorageBuffers[inDstBinding]->GetStorageMappedMemoryRegion();
    memcpy(memory, &data, sizeof(T));
}

inline void
Uniform3D::UpdateUniformBuffer(int inDstBinding, void** inData, size_t inSize) {
    void* memory =
         mUniformBuffers[inDstBinding]->GetUniformMappedMemoryRegion();
...
inline void
Uniform3D::UpdateStorageBuffer(int inDstBinding, void** inData, size_t inSize) {
    void* memory =
         mStorageBuffers[inDstBinding]->GetStorageMappedMemoryRegion();
    memcpy(memory, *inData, inSize);
}
