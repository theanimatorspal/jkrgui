...
#else
            mainState.script_file(OptionRun);
#endif
      } else {
...
int c(const std::string &str, const std::string &sub) {
      ...
      for (int offset = str.find(sub); offset != std::string::npos;) {
            if (isalpha(sub[0])) {
                  ...
                  int right          = ((offset + sub.length()) >= str.length()) ? str.length()
                                                                                 : (offset + sub.length());
...
void ProcessCmdLine(int ArgCount, char **ArgStrings) {
      ...
      auto Update = []() {
            ...
            filesystem::copy(
                 src / "app.lua", dest / "app.lua", filesystem::copy_options::skip_existing);
            filesystem::copy(src / "res",
                             dest / "res",
                             filesystem::copy_options::recursive |
                                  filesystem::copy_options::skip_existing);
            filesystem::copy(src / ".vscode",
                             dest / ".vscode",
                             filesystem::copy_options::recursive |
                                  filesystem::copy_options::skip_existing);
      };
