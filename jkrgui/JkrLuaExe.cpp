...
bool FlagLuaLibraryEnvironment = false;
...
extern umap<s, v<s>> CommandLine(int ArgCount,
                                 char **ArgStrings); // TODO Complete This
...
void RunScript() {
  ...
  if (OptionRun != "") {
#ifndef ANDROID
    Jkr.set_function("CreateAndroidEnvironment",
    ...
    if (std::filesystem::exists("main.jkr")) {
      ...
      for (int i = 1; i <= fcount; ++i) {
        sol::basic_bytecode<> code =
            File.Read<decltype(code)>(s("func") + std::to_string(i));
...
static bool IsBoundary(char c) {
  return std::isspace(c) or std::ispunct(c) or c == '\0';
}
...
int c(const std::string &str, const std::string &sub) {
  ...
  for (int offset = str.find(sub); offset != std::string::npos;) {
    if (isalpha(sub[0])) {
      ...
      int right = ((offset + sub.length()) >= str.length())
                      ? str.length()
                      : (offset + sub.length());
...
void ProcessCmdLine(int ArgCount, char **ArgStrings) {
  ...
  app.add_flag("--lib-env", FlagLuaLibraryEnvironment);
  ///
  ...
  app.add_option("--builddir", OptionAndroidBuildDirName,
                 "Android Build Directory Name");
  app.add_option("--run", OptionRun, "Run File, Defaults to app.lua");

  app.parse(ArgCount, ArgStrings);

  if (FlagLuaLibraryEnvironment) {
    BuildSystem::CreateLuaLibraryEnvironment(OptionAndroidAppName, "cpp", true);
  }
  ...
  auto Update = []() {
    ...
    // filesystem::copy(src / "bundle.lua", dest / "bundle.lua",
    // filesystem::copy_options::update_existing);
    filesystem::copy(src / "app.lua", dest / "app.lua",
                     filesystem::copy_options::skip_existing);
    filesystem::copy(src / "res", dest / "res",
                     filesystem::copy_options::recursive |
                         filesystem::copy_options::skip_existing);
    filesystem::copy(src / ".vscode", dest / ".vscode",
                     filesystem::copy_options::recursive |
                         filesystem::copy_options::skip_existing);
  };
  if (FlagGenerate) {
  ...
  if (FlagRepl) {
    ...
    std::filesystem::current_path(std::filesystem::path(getenv("JKRGUI_DIR")) /
                                  "luaproject");
    ...
    auto ReplLoop = [&]() {
      while (true) {
        ...
        while (getline(cin, line)) {
          ...
          scope_end +=
              c(line, "end") + c(line, ")") + c(line, "}") + c(line, "]]");
