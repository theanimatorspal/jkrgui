...
template <> class PainterParameter<PainterParameterContext::StorageBuffer> : public PainterParameterBase {
       public:
               ...
               void Setup(vk::DeviceSize inStorageBufferSize) { PainterParameterBase::Setup(mStorageBufferPtr, inStorageBufferSize); }
               void SetupCoherent(vk::DeviceSize inStorageBufferSize) { PainterParameterBase::Setup(mStorageBufferCoherentPtr, inStorageBufferSize, &mStorageBufferCoherentMemoryMappedRegion); }
               void Register(vk::DeviceSize inOffset, uint32_t inDstBinding, uint32_t inDstArrayElement, VulkanDescriptorSet& inDescriptorSet) {
                              mVulkanDescriptorSetHandler.RW<BufferContext::Storage>(inDescriptorSet, *mStorageBufferPtr, inOffset, inDstBinding, inDstArrayElement);
               }
               void RegisterCoherent(vk::DeviceSize inOffset, uint32_t inDstBinding, uint32_t inDstArrayElement, VulkanDescriptorSet& inDescriptorSet) {
                              mVulkanDescriptorSetHandler.RW<BufferContext::Storage>(inDescriptorSet, *mStorageBufferCoherentPtr, inOffset, inDstBinding, inDstArrayElement);
               }
...
template <> class PainterParameter<PainterParameterContext::UniformBuffer> : public PainterParameterBase {
       public:
               ...
               void Register(vk::DeviceSize inOffset, uint32_t inDstBinding, uint32_t inDstArrayElement, VulkanDescriptorSet& inDescriptorSet) {
                              mVulkanDescriptorSetHandler.RW<BufferContext::Uniform>(inDescriptorSet, *mUniformBufferPtr, inOffset, inDstBinding, inDstArrayElement);
               }
...
template <> class PainterParameter<PainterParameterContext::StorageImage> : public PainterParameterBase {
       public:
               ...
               void Setup(uint32_t inWidth, uint32_t inHeight) { PainterParameterBase::Setup(mSampler, mStorageImagePtr, inWidth, inHeight); }
               void Register(vk::DeviceSize inOffset, uint32_t inDstBinding, uint32_t inDstArrayElement, VulkanDescriptorSet& inDescriptorSet) {
                              mVulkanDescriptorSetHandler.RW<ImageContext::Storage>(inDescriptorSet, *mStorageImagePtr, *mSampler, inDstBinding, inDstArrayElement);
               }
...
template <> class PainterParameter<PainterParameterContext::UniformImage> : public PainterParameterBase {
       public:
               ...
               void Setup(const std::string_view inFileName) { PainterParameterBase::Setup(mSampler, mUniformImagePtr, inFileName); }
               void Setup(void** inData, uint32_t inWidth, uint32_t inHeight, uint32_t inChannelCount) {
                              PainterParameterBase::Setup(mSampler, mUniformImagePtr, inData, inWidth, inHeight, inChannelCount);
               }
