...
inline void PainterParameterBase::SetupImage(Up<VulkanSampler>& inUniformImageSampler, Up<T>& inUniformImage, std::span<const sv> inFileNames) {
          ...
          inUniformImage->SubmitImmediateCmdCopyFromData(
           mInstance.GetGraphicsQueue(), mInstance.GetUtilCommandBuffer(), mInstance.GetDevice(), Channels * Height * Width, Datas_ptr);
...
template <> class PainterParameter<PainterParameterContext::StorageImage> : public PainterParameterBase {
      public:
          PainterParameter(const Instance& inInstance) : PainterParameterBase(inInstance) {}
          inline void Setup(uint32_t inWidth, uint32_t inHeight) { PainterParameterBase::Setup(mSampler, mStorageImagePtr, inWidth, inHeight); }
...
template <> class PainterParameter<PainterParameterContext::UniformImage> : public PainterParameterBase {
      public:
          ...
          inline void Setup(void** inData, uint32_t inWidth, uint32_t inHeight, uint32_t inChannelCount) {
