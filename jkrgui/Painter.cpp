...
Jkr::Painter::Painter(const Instance& inInstance, const Window& inWindow, const PainterCache& inCache, PipelineContext inPipelineContext)
...
void Painter::OptimizeParameter(const Instance& inInstance, const PainterParameter<PainterParameterContext::StorageImage>& inImage, const Window& inWindow) {
...
void Painter::RegisterPainterParameter<PainterParameterContext::StorageBuffer>(
          const PainterParameter<PainterParameterContext::StorageBuffer>& inPainterParameter, vk::DeviceSize inOffset, uint32_t inDstBinding, uint32_t inDstArrayElement, RegisterMode inRegisterMode) {
               if (inRegisterMode != RegisterMode::ComputeOnly)
                              mDescriptorUpdateHandler.RW<BufferContext::Storage>(mVertexFragmentDescriptorSet, inPainterParameter.GetStorageBuffer(), inOffset, inDstBinding, inDstArrayElement);

               if (inRegisterMode == RegisterMode::AllShaders or inRegisterMode == RegisterMode::ComputeOnly)
                              mDescriptorUpdateHandler.RW<BufferContext::Storage>(mComputeDescriptorSet, inPainterParameter.GetStorageBuffer(), inOffset, inDstBinding, inDstArrayElement);
}

template <>
void Painter::RegisterPainterParameter<PainterParameterContext::UniformBuffer>(
          const PainterParameter<PainterParameterContext::UniformBuffer>& inPainterParameter, vk::DeviceSize inOffset, uint32_t inDstBinding, uint32_t inDstArrayElement, RegisterMode inRegisterMode) {
               if (inRegisterMode != RegisterMode::ComputeOnly)
                              mDescriptorUpdateHandler.RW<BufferContext::Uniform>(mVertexFragmentDescriptorSet, inPainterParameter.GetUniformBuffer(), inOffset, inDstBinding, inDstArrayElement);
               if (inRegisterMode == RegisterMode::AllShaders or inRegisterMode == RegisterMode::ComputeOnly)
                              mDescriptorUpdateHandler.RW<BufferContext::Uniform>(mComputeDescriptorSet, inPainterParameter.GetUniformBuffer(), inOffset, inDstBinding, inDstArrayElement);
}

template <>
void Painter::RegisterPainterParameter<PainterParameterContext::StorageImage>(
          const PainterParameter<PainterParameterContext::StorageImage>& inPainterParameter, vk::DeviceSize inOffset, uint32_t inDstBinding, uint32_t inDstArrayElement, RegisterMode inRegisterMode) {
               if (inRegisterMode != RegisterMode::ComputeOnly)
                              mDescriptorUpdateHandler.RW<ImageContext::Storage>(
                                        mVertexFragmentDescriptorSet, inPainterParameter.GetStorageImage(), inPainterParameter.GetStorageImageSampler(), inDstBinding, inDstArrayElement);
               if (inRegisterMode == RegisterMode::AllShaders or inRegisterMode == RegisterMode::ComputeOnly)
                              mDescriptorUpdateHandler.RW<ImageContext::Storage>(
                                        mComputeDescriptorSet, inPainterParameter.GetStorageImage(), inPainterParameter.GetStorageImageSampler(), inDstBinding, inDstArrayElement);
}

template <>
void Painter::RegisterPainterParameter<PainterParameterContext::UniformImage>(
          const PainterParameter<PainterParameterContext::UniformImage>& inPainterParameter, vk::DeviceSize inOffset, uint32_t inDstBinding, uint32_t inDstArrayElement, RegisterMode inRegisterMode) {
               if (inRegisterMode != RegisterMode::ComputeOnly)
                              mDescriptorUpdateHandler.RW<ImageContext::Default>(
                                        mVertexFragmentDescriptorSet, inPainterParameter.GetUniformImage(), inPainterParameter.GetUniformImageSampler(), inDstBinding, inDstArrayElement);
               if (inRegisterMode == RegisterMode::AllShaders or inRegisterMode == RegisterMode::ComputeOnly)
                              mDescriptorUpdateHandler.RW<ImageContext::Default>(
                                        mComputeDescriptorSet, inPainterParameter.GetUniformImage(), inPainterParameter.GetUniformImageSampler(), inDstBinding, inDstArrayElement);
}

template <>
void Painter::RegisterPainterParameter<PainterParameterContext::SkyboxImage>(
          const PainterParameter<PainterParameterContext::SkyboxImage>& inPainterParameter, vk::DeviceSize inOffset, uint32_t inDstBinding, uint32_t inDstArrayElement, RegisterMode inRegisterMode) {
               if (inRegisterMode != RegisterMode::ComputeOnly)
                              mDescriptorUpdateHandler.RW<ImageContext::Default>(
                                        mVertexFragmentDescriptorSet, inPainterParameter.GetUniformImage(), inPainterParameter.GetUniformImageSampler(), inDstBinding, inDstArrayElement);
               if (inRegisterMode == RegisterMode::AllShaders or inRegisterMode == RegisterMode::ComputeOnly)
...
template <>
void Painter::RegisterPainterParameter<PainterParameterContext::UniformSampler>(const PainterParameter<PainterParameterContext::UniformSampler>& inPainterParameter,
                                                                                vk::DeviceSize inOffset,
                                                                                uint32_t inDstBinding,
                                                                                uint32_t inDstArrayElement,
                                                                                RegisterMode inRegisterMode) {
               if (inRegisterMode != RegisterMode::ComputeOnly) mDescriptorUpdateHandler.RW(mVertexFragmentDescriptorSet, inPainterParameter.GetSampler(), inDstBinding, inDstArrayElement);
               if (inRegisterMode == RegisterMode::AllShaders or inRegisterMode == RegisterMode::ComputeOnly)
                              mDescriptorUpdateHandler.RW(mComputeDescriptorSet, inPainterParameter.GetSampler(), inDstBinding, inDstArrayElement);
}
...
void Painter::BindDispatchParameters_EXT(const Window& inWindow, CmdParam inParam) {
               ...
               Cmd.GetCommandBufferHandle().bindDescriptorSets(
                         vk::PipelineBindPoint::eCompute, mGUIPainterCache.GetComputePipelineLayout().GetPipelineLayoutHandle(), 0, mComputeDescriptorSet.GetDescriptorSetHandle(), {});
...
void Painter::BindDrawParamters_EXT(const Primitive& inPrimitive, const Window& inWindow, CmdParam inParam) {
               ...
               Cmd.GetCommandBufferHandle().bindDescriptorSets(
                         vk::PipelineBindPoint::eGraphics, mGUIPainterCache.GetVertexFragmentPipelineLayout().GetPipelineLayoutHandle(), 0, mVertexFragmentDescriptorSet.GetDescriptorSetHandle(), {});
