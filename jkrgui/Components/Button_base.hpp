...
public:
    Button_base(_2d& inR, EventManager& inE)
        : Area_base(inR, inE)
    {
    }
    constexpr void Load()
...
public:
    SETTER SetColor(glm::vec4 inColor) { mCurrentColor = inColor; }
    SETTER SetNormalColor(glm::vec4 inColor) { mButtonNormalColor = inColor; }
    SETTER SetHoveredColor(glm::vec4 inColor) { mButtonHoveredColor = inColor; }
    SETTER SetClickedColor(glm::vec4 inColor) { mButtonClickedColor = inColor; }
