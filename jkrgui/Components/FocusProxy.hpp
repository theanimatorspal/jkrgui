...
namespace Jkr::Component
{
	...
	protected:
		void FocusEvent(Area_base& inAreaBase, bool isLeftButtonPressed, bool isMouseOver, bool isMouseOnTop)
		{
			...
			{
				if (isLeftButtonPressed and isMouseOver and !inAreaBase.IsFocused())  inAreaBase.ToggleFocus();
				if (inAreaBase.IsFocused() and !isMouseOver and isLeftButtonPressed) inAreaBase.ToggleFocus();
			}
			else {
				if (isMouseOver and !inAreaBase.IsFocused())  inAreaBase.ToggleFocus();
				if (inAreaBase.IsFocused() and !isMouseOver) inAreaBase.ToggleFocus();
			}

			if (inAreaBase.IsFocused()) { mOutLineColor = mOutLineColorFocused; }
			else { mOutLineColor = mOutLineColorUnFocused; }
		}

		void FocusEvent(Area_base& inAreaBase, bool isLeftButtonPressed)
		{
			if (isLeftButtonPressed and inAreaBase.IsMouseWithin() and !inAreaBase.IsFocused())  inAreaBase.ToggleFocus();
			if (inAreaBase.IsFocused() and !inAreaBase.IsMouseWithin() and isLeftButtonPressed) inAreaBase.ToggleFocus();
