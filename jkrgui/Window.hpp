...
class Window : public SDLWindow {
       public:
               ...
               using FrameBufferType =
                         VulkanFrameBuffer<RenderPassContext::MSAA, 3, VulkanImage<ImageContext::ColorAttach>, VulkanImage<ImageContext::DepthImage>, VulkanImage<ImageContext::ExternalHandled>>;
...
SETTER Jkr::Window::SetScissor(int inX, int inY, int inW, int inH, ParameterContext inContext) {
               GetCommandBuffers(inContext)[mCurrentFrame].GetCommandBufferHandle().setScissor(0, vk::Rect2D{vk::Offset2D{inX, inY}, vk::Extent2D{(ui)inW, (ui)inH}});
}
