...
class VulkanImageBase {
       public:
               ...
               ~VulkanImageBase();
               void SubmitImmediateCmdCopyFromData(
       ...
       public:
               template <ImageContext inImageContext> void FillImageProperties(uint32_t inNumSamples = 1);
...
namespace ksai {
template <ImageContext inImageContext> inline void VulkanImageBase::FillImageProperties(uint32_t inNumSamples) {
               ...
               if constexpr (inImageContext == ImageContext::DepthImage) {
                              ...
                              mImageProperties.mImageUsage  = vk::ImageUsageFlagBits::eStorage | vk::ImageUsageFlagBits::eTransferSrc;
...
namespace ksai {
template <ImageContext inImageContext> inline void VulkanImage<inImageContext>::MoveMembers(ksai::VulkanImage<inImageContext>& Other) {
...
template <ImageContext inImageContext> inline VulkanImage<inImageContext>::VulkanImage(const VulkanDevice& inDevice) : VulkanImageBase(inDevice) {
               FillImageProperties<inImageContext>();
               CreateImageAndBindMemory(mImage, mDeviceMemory);
               CreateImageView(mImage);
}

template <ImageContext inImageContext> inline VulkanImage<inImageContext>::VulkanImage(const VulkanDevice& inDevice, ui inWidth, ui inHeight) : VulkanImageBase(inDevice) {
               FillImageProperties<inImageContext>();
...
template <ImageContext inImageContext> inline VulkanImage<inImageContext>::VulkanImage(const VulkanDevice& inDevice, const VulkanSurface& inSurface, ui inMSAASamples) : VulkanImageBase(inDevice) {
...
inline VulkanImage<inImageContext>::VulkanImage(const VulkanDevice& inDevice, const VulkanSurface& inSurface, const vk::Image& inImage, vk::ImageView& inImageView) : VulkanImageBase(inDevice) {
               mImageProperties.mExtent = inSurface.GetExtent();
               mImage                   = inImage;
