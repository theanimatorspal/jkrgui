...
VulkanQueueContext::VulkanQueueContext(const VulkanInstance& inInstance, const VulkanPhysicalDevice& inDevice) : mPhysicalDevice(inDevice.GetPhysicalDeviceHandle()) {
               ...
               auto PropertyIterator  = std::find_if(
                         mQueueFamilyProperties.begin(), mQueueFamilyProperties.end(), [](vk::QueueFamilyProperties const& qfp) { return (qfp.queueFlags & vk::QueueFlagBits::eGraphics); });
               ...
               size_t presentQueueFamilyIndex = mPhysicalDevice.getSurfaceSupportKHR(mGraphicsQueueIndex, SurfaceHandle) ? mGraphicsQueueIndex : mQueueFamilyProperties.size();
               ...
               if (presentQueueFamilyIndex == mQueueFamilyProperties.size()) {
                              for (int i = 0; i < mQueueFamilyProperties.size(); i++) {
                                             bool isQueueSupportedForPresentAndGraphics = (mQueueFamilyProperties[i].queueFlags & vk::QueueFlagBits::eGraphics) &&
