...
template <>
VulkanDescriptorSetLayout<NoOfShaderModules, ShaderModuleContext::Vertex, ShaderModuleContext::Fragment>::VulkanDescriptorSetLayout(const VulkanDevice& inDevice, const std::array<VulkanShaderModule, NoOfShaderModules>& inModules)
...
template <>
VulkanDescriptorSetLayout<1, ShaderModuleContext::Compute>::VulkanDescriptorSetLayout(const VulkanDevice& inDevice, const std::array<VulkanShaderModule, 1>& inModules)
    : VulkanDescriptorSetLayoutBase(inDevice)
    , mModules(inModules)
{
    std::vector<vk::DescriptorSetLayoutBinding> DescriptorSetLayoutBindings;
    ...
    , mModules(inModules)
{
    ...
    Flags[Flags.size() - 1] = vk::DescriptorBindingFlagBits::eVariableDescriptorCount | vk::DescriptorBindingFlagBits::ePartiallyBound | vk::DescriptorBindingFlagBits::eUpdateUnusedWhilePending;
...
template <>
VulkanDescriptorSetLayout<1, ShaderModuleContext::Compute>::VulkanDescriptorSetLayout(const VulkanDevice& inDevice, const std::array<VulkanShaderModule, 1>& inModules, uint32_t inVariableSizedDescriptorMaxCount)
    : VulkanDescriptorSetLayoutBase(inDevice)
    , mModules(inModules)
{
    std::vector<vk::DescriptorSetLayoutBinding> DescriptorSetLayoutBindings;
...
void ksai::VulkanDescriptorSetLayoutBase::FillDescriptorLayoutBindings(const spirv_cross::ShaderResources& inResources, const spirv_cross::Compiler& inGlslComp, std::vector<vk::DescriptorSetLayoutBinding>& DescriptorSetLayoutBindings)
{
...
{
    auto SearchIndex = std::find_if(inDescriptorSetLayoutBindings.begin(), inDescriptorSetLayoutBindings.end(),
