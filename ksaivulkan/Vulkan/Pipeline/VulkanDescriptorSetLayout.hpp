...
namespace ksai {
	...
	inline void VulkanDescriptorSetLayoutBase::FillDescriptorLayoutBindings(const spirv_cross::ShaderResources& inResources, const spirv_cross::Compiler& inGlslCompiler, std::vector<vk::DescriptorSetLayoutBinding>& inDescriptorSetLayoutBindings)
	{
		...
		if constexpr (Context == ShaderModuleContext::Vertex) ShaderStageFlag = vk::ShaderStageFlagBits::eVertex;
		else if constexpr (Context == ShaderModuleContext::Fragment) ShaderStageFlag = vk::ShaderStageFlagBits::eFragment;
		else if constexpr (Context == ShaderModuleContext::Compute) ShaderStageFlag = vk::ShaderStageFlagBits::eCompute;
		{
