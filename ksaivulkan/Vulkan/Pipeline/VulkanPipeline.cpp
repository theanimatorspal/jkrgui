...
void VulkanPipelineBase::FillVertexInputDescriptions(
     ...
     std::vector<vk::VertexInputAttributeDescription>& InputAttrDescription) {
    {
        ...
        for (auto i = 0; i < Resources.stage_inputs.size(); i++) {
            ...
            uint32_t location =
                 comp.get_decoration(u.id, spv::DecorationLocation);
            ...
            switch (siz) {
                case 1:
                    ...
                    AttributeSize = static_cast<size_t>(1) *
                ...
                case 2:
                    ...
                    AttributeSize = static_cast<size_t>(2) *
                                    static_cast<uint32_t>(sizeof(float));
                    break;
                case 3:
                    if (type.basetype == spirv_cross::SPIRType::Int)
                    ...
                    AttributeSize = static_cast<size_t>(3) *
                                    static_cast<uint32_t>(sizeof(float));
                    break;
                case 4:
                    if (type.basetype == spirv_cross::SPIRType::Int)
                    ...
                    AttributeSize = static_cast<size_t>(4) *
                                    static_cast<uint32_t>(sizeof(float));
                    break;
            }
        ...
        VertexInputBindingDesp.push_back(vk::VertexInputBindingDescription(
             FirstBindingIndex, InputBindingWholeSize));
    }
}
...
void VulkanPipelineBase::BuildPipeline(
     VulkanPipelineCache& inCache,
     const VulkanPipelineContextBase& inContext,
     const VulkanRenderPassBase& inRenderPass,
     const VulkanPipelineLayoutBase& inLayout,
    ...
    if (inPipelineContext == PipelineContext::Compute) {
        ...
        auto Result = mDevice.createComputePipeline(
             inCache.GetPipelineCacheHandle(), ComputePipelineCreateInfo);
        mPipeline = Result.value;
    } else {
        auto& VertexShaderCompiler =
             inModules[0].GetShaderResourcesCompilerHandle();
        ...
        mPipeline =
             mDevice.createGraphicsPipeline(nullptr, GraphicsPipelineCreateInfo)
                  .value;
    }
}
