...
ksai::VulkanImageBase::VulkanImageBase(const VulkanDevice& inDevice, bool inDestroyImageView)
    : mPhysicalDevice(inDevice.GetPhysicalDeviceHandle()), mDevice(inDevice.GetDeviceHandle()), mDestroyImageView(inDestroyImageView) {}
...
void ksai::VulkanImageBase::SubmitImmediateCmdCopyFromData(
          const VulkanQueue<QueueContext::Graphics>& inQueue, const VulkanCommandBuffer& inCmdBuffer, const VulkanDevice& inDevice, void** inData, vk::DeviceSize inSize) {
...
void VulkanImageBase::SubmitImmediateCmdCopyFromData(
          const VulkanQueue<QueueContext::Graphics>& inQueue, const VulkanCommandBuffer& inCmdBuffer, const VulkanDevice& inDevice, vk::DeviceSize inSize, std::span<void**> inLayerImageDatas) {
               vk::DeviceSize size = inLayerImageDatas.size() * inSize;
               VulkanBuffer<BufferContext::Staging, MemoryType::HostVisibleAndCoherenet> StagingBuffer(inDevice, size);
               void* MapRegion;
               StagingBuffer.MapMemoryRegion(&MapRegion);
...
void ksai::VulkanImageBase::CmdTransitionImageLayout(const VulkanCommandBuffer& inBuffer,
               ...
               auto ImgMemBarr =
                         vk::ImageMemoryBarrier(inBeforeAccess, inAfterAccess, inOldImageLayout, inNewImageLayout, VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED, mImage, ImageSubResourceRange);
...
void ksai::VulkanImageBase::CreateImageView(vk::Image& inImage) {
               ...
               vk::ImageViewCreateInfo ImageViewCreateInfo(vk::ImageViewCreateFlags(), inImage, mImageProperties.mImageViewType, mImageProperties.mImageFormat, {}, ImageSubResourcesRange);

               mImageView = mDevice.createImageView(ImageViewCreateInfo);
}

void ksai::VulkanImageBase::CmdCopyImageFromImageAfterStage(const VulkanQueue<QueueContext::Graphics>& inQueue,
               ...
               vk::ImageSubresourceLayers DstSubResource(DstImageProp.mImageAspect, 0, 0, DstImageProp.mArrayLayers);
               vk::ImageSubresourceRange SrcSubResourceRange(SrcSubResource.aspectMask, 0, 0, 0, SrcImageProp.mArrayLayers);
               vk::ImageSubresourceRange DstSubResourceRange(DstSubResource.aspectMask, 0, 0, 0, DstImageProp.mArrayLayers);
               vk::ImageCopy CopyRegion(SrcSubResource, vk::Offset3D{}, DstSubResource, vk::Offset3D{}, vk::Extent3D(SrcVulkanImage.GetImageExtent(), 0));
