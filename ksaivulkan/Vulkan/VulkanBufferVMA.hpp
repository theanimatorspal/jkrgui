...
public:
    inline constexpr void MapMemoryRegion(void** outMappedMemoryRegion)
        requires(inBufferStorageType == MemoryType::HostVisibleAndCoherenet || inBufferStorageType == MemoryType::HostVisibleAndHostCached);
    inline constexpr void UnMapMemoryRegion()
        requires(inBufferStorageType == MemoryType::HostVisibleAndCoherenet);
    inline constexpr void FlushMemoryRanges(vk::DeviceSize inOffset, vk::DeviceSize inSize)
...
namespace ksai {
template <BufferContext inBufferContext, MemoryType inBufferStorageType>
inline constexpr void VulkanBufferVMA<inBufferContext, inBufferStorageType>::MapMemoryRegion(void** outMappedMemoryRegion)
    requires(inBufferStorageType == MemoryType::HostVisibleAndCoherenet || inBufferStorageType == MemoryType::HostVisibleAndHostCached)
{
    vmaMapMemory(mAllocator.GetVMAHandle(), mAllocation, outMappedMemoryRegion);
}
template <BufferContext inBufferContext, MemoryType inBufferStorageType>
inline constexpr void VulkanBufferVMA<inBufferContext, inBufferStorageType>::UnMapMemoryRegion()
    requires(inBufferStorageType == MemoryType::HostVisibleAndCoherenet)
{
    vmaUnmapMemory(mAllocator.GetVMAHandle(), mAllocation);
}
    ...
    , mAllocator(inVMA)
{
    ...
    if constexpr (inBufferStorageType == MemoryType::HostVisibleAndCoherenet)
        AllocationCreateInfo_C.flags = AllocationCreateInfo_C.flags | VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT;
    else if constexpr (inBufferStorageType == MemoryType::HostVisibleAndHostCached) {
