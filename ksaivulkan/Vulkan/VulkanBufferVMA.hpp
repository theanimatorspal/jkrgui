...
namespace ksai {
	...
	public:
		...
		SETTER FlushMemoryRanges(vk::DeviceSize inOffset, vk::DeviceSize inSize) requires(inBufferStorageType == MemoryType::HostVisibleAndHostCached)
		{
	...
	public:
		...
		~VulkanBufferVMA()
		{
			if constexpr (inBufferStorageType == MemoryType::HostVisibleAndCoherenet or inBufferStorageType == MemoryType::HostVisibleAndHostCached)
			{
				if(mMemoryMapped)
...
namespace ksai {
	template <BufferContext inBufferContext, MemoryType inBufferStorageType>
	SETTER VulkanBufferVMA<inBufferContext, inBufferStorageType>::MapMemoryRegion(void** outMappedMemoryRegion) 
	{
		mMemoryMapped = true;
