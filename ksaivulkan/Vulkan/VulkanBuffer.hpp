...
class VulkanBufferBase {
       public:
               ...
               void CmdCopyFromImage(const VulkanCommandBuffer& inCmdBuffer,
       ...
       protected:
               template <BufferContext inBufferContext, MemoryType inBufferStorageType> constexpr void FillBufferUsage(vk::BufferCreateInfo& inInfo);
...
namespace ksai {
template <BufferContext inBufferContext, MemoryType inBufferStorageType> class VulkanBuffer : public VulkanBufferBase {
       public:
...
namespace ksai {
template <BufferContext inBufferContext, MemoryType inBufferStorageType> inline constexpr void VulkanBufferBase::FillBufferUsage(vk::BufferCreateInfo& inInfo) {
               if constexpr (inBufferContext == BufferContext::Vertex)
                              inInfo.setUsage(vk::BufferUsageFlagBits::eVertexBuffer | vk::BufferUsageFlagBits::eTransferDst);
               else if constexpr (inBufferContext == BufferContext::Index)
                              inInfo.setUsage(vk::BufferUsageFlagBits::eIndexBuffer | vk::BufferUsageFlagBits::eTransferDst);
               else if constexpr (inBufferContext == BufferContext::Uniform)
                              inInfo.setUsage(vk::BufferUsageFlagBits::eUniformBuffer | vk::BufferUsageFlagBits::eTransferDst);
               else if constexpr (inBufferContext == BufferContext::Storage)
                              inInfo.setUsage(vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eTransferDst);
               else if constexpr (inBufferContext == (BufferContext::Vertex | BufferContext::Storage))
                              inInfo.setUsage(vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eVertexBuffer | vk::BufferUsageFlagBits::eTransferDst);
                              ...
                              inInfo.setUsage(vk::BufferUsageFlagBits::eStorageBuffer | vk::BufferUsageFlagBits::eUniformBuffer | vk::BufferUsageFlagBits::eTransferDst);
}

template <BufferContext Context> inline void VulkanBufferBase::Bind(const VulkanCommandBuffer& inCmdBuffer) const {
               if constexpr (Context == BufferContext::Vertex)
                              ...
                              inCmdBuffer.GetCommandBufferHandle().bindIndexBuffer(mBufferHandle, 0, vk::IndexType::eUint32);
}
} // namespace ksai

namespace ksai {
template <BufferContext inBufferContext, MemoryType inBufferStorageType>
inline constexpr void VulkanBuffer<inBufferContext, inBufferStorageType>::MapMemoryRegion(void** outMappedMemoryRegion)
               requires(inBufferStorageType == MemoryType::HostVisibleAndCoherenet)
{
...
template <BufferContext inBufferContext, MemoryType inBufferStorageType>
inline VulkanBuffer<inBufferContext, inBufferStorageType>::VulkanBuffer(const VulkanDevice& inDevice, size_t inSize) : VulkanBufferBase(inDevice) {
               mSize = inSize;
               ...
               if constexpr (inBufferStorageType == MemoryType::DeviceLocal)
                              ...
                              MemoryPropertyFlagBits = vk::MemoryPropertyFlagBits::eHostVisible | vk::MemoryPropertyFlagBits::eHostCoherent;
...
template <BufferContext inBufferContext, MemoryType inBufferStorageType> inline VulkanBuffer<inBufferContext, inBufferStorageType>::~VulkanBuffer() {
